...
function area1() {
    if (base1.value !=='' && height1.value !== ''){
        ...
        output[0].innerHTML = `<span style="color : green"> The area of triangle whose base ${base1.value} and height ${height1.value} is ${area.toFixed(2)} </span>`;    
...
function area2 ( ){
    if (lengths[0].value !=='' && lengths[1].value !== '' && lengths[2].value !== ''){
        ...
        output[1].innerHTML = `<span style="color : green">The area of triangle with sides ${lengths[0].value}, ${lengths[1].value}, ${lengths[2].value} is ${area.toFixed(2)}</span>`;
